...
Under App Sandbox the live database is inside the container:
...
The pre-sandbox location `~/Library/userlexicon.sqlite3` is now only the one-time migration source (`UserLexicon.migrateLegacyLexiconIfNeeded`) — see `docs/2026-07-18-sandboxing.md`.

### Switch Character Standard

Under App Sandbox, UserDefaults live in the container, so `defaults write hk.eduhk.inputmethod.Prompt …` (the global domain) is **not** read by the app — target the container's preference plist by path:

```bash
PREFS=~/Library/Containers/hk.eduhk.inputmethod.Prompt/Data/Library/Preferences/hk.eduhk.inputmethod.Prompt
defaults write "$PREFS" CharacterStandard -int 4  # Simplified
defaults write "$PREFS" CharacterStandard -int 1  # Traditional
osascript -e 'tell application id "hk.eduhk.inputmethod.Prompt" to quit'
```
...
* **`userlexicon.sqlite3`** — Under App Sandbox lives in the container at `~/Library/Containers/hk.eduhk.inputmethod.Prompt/Data/Library/userlexicon.sqlite3` (migrated once from the pre-sandbox `~/Library/userlexicon.sqlite3`), managed by `UserLexicon`. Table: `userlexicontable` (id, frequency, word, romanization, shortcut, ping).
...
* `PromptInputController.swift` — `IMKInputController` subclass. All key handling runs in `Task { @MainActor in ... }`. Core state: `bufferText`, `candidates`, `selectedCandidates`, `wordCreationCharacters`/`wordCreationPinyins`/`wordCreationInputs`, `isPunctuationFullWidth`, `inputStage`, `inputForm`, `isIntendingToRecord`. Holds `static let sharedVoiceRecorder`, `static var voiceModelObserver`, and `static let sharedAppContext` (see "Multi-instance controller invariants" below).

* `UserLexicon.swift` — Stores in the sandbox container (`…/Data/Library/userlexicon.sqlite3`, resolved via `URL.libraryDirectory`; on first sandboxed launch `migrateLegacyLexiconIfNeeded` copies the pre-sandbox `~/Library/userlexicon.sqlite3` in). Has prepared statements for ping/shortcut/find queries. `handle(_:)` boosts selected word by +1000 and decays same-pinyin siblings by 10% (`frequency * 9 / 10`, min 1). All selections (including first candidate) are recorded so frequency reflects long-term usage proportions. Initial frequency: 1000.
...
* `AppSettings.swift` — Persistent settings in UserDefaults (page size, input memory on/off, `defaultInputModeOnActivation`, etc.). For voice it holds `isVoiceRecognitionEnabled` (persisted toggle) and `voiceModelLoadState` (runtime, updated by `VoiceRecorder`). `senseVoiceModelDir` is **computed, not persisted**: it returns the bundled model directory (`bundledSenseVoiceModelURL`, `Resources/SenseVoiceModel/`) only when the toggle is on AND the model is actually bundled; otherwise `""`. `isVoiceModelBundled` reports whether this build shipped the model.

* `VoiceRecorder.swift` — Captures 16kHz mono Float32 PCM audio via `AVAudioEngine` (`AudioCapture`) and transcribes via the in-process `SenseVoiceEngine` (Core ML). The model directory is configured in Settings; transcription result is inserted as text via `onTranscription` callback. See "Voice Recognition Feature" below.

* `SenseVoiceEngine.swift` / `Fbank.swift` — Pure-Swift in-process SenseVoiceSmall inference (Core ML + Accelerate). The model is **bundled into the app at build time** (a Run Script phase copies `SenseVoiceSmall/dist/*` into `Resources/SenseVoiceModel/`) and loaded from `AppSettings.senseVoiceModelDir`, which resolves to that bundle directory. See "Voice Recognition Feature" below and `SenseVoiceSmall/README.md`.
...
### Voice Recognition Feature (SenseVoiceSmall)

Triggered by **Shift+Space when NOT buffering** (idle/standby in Mandarin mode) and only if the SenseVoice model is loaded.

Recognition runs **in-process** via a local Core ML SenseVoiceSmall model (non-autoregressive encoder + single CTC head). The whole pipeline is **pure Swift** — no third-party runtime, no Python. See `SenseVoiceSmall/README.md` for the model, the one-click build script (`build_sensevoice_mlmodelc.sh`), and the full pipeline write-up.
...
2. Audio is streamed in real time via `AVAudioEngine` (`AudioCapture`); 16kHz mono Float32 samples accumulate in memory
3. `Space` keyUp (or Shift release + subsequent Space keyUp) → `stopRecording()` (plays "Pop" sound), runs SenseVoice transcription on `voiceQueue`
...
**Empty / hallucination filtering** (in `stopRecording`, before transcription): recordings shorter than 8000 samples (~0.5s) or with RMS ≤ 0.001 are skipped and fire `onTranscription("")`. SenseVoice has no whisper-style no-speech-prob.

**Engine (`VoiceRecorder.swift` + `SenseVoiceEngine.swift` + `Fbank.swift`):**

* `SenseVoiceEngine` — `public final class`; `transcribe(samples:[Float]) -> String`. Pipeline: Kaldi fbank(80, `Fbank.swift`, Accelerate/vDSP; mel-energy floor = `Float.ulpOfOne`, natural log; vDSP zrip power ×0.25) → LFR(7,6)→560 → CMVN (`am.mvn`) → prepend 4 query embeddings (`query_embeddings.f32`) → ×√512 → sinusoidal pos-enc → Core ML predict → CTC greedy → `tokens.json` detokenize → strip `<|...|>` tags. **Reads the Core ML output with the MLMultiArray's real `strides`** — CoreML pads the time dim to an aligned stride (e.g. 171→192); assuming contiguous memory yields garbage.
* `VoiceRecorder` loads `SenseVoiceEngine` from `AppSettings.senseVoiceModelDir` (the bundled directory) on `voiceQueue`; `reload()` is idempotent. Loading is **driven at the app level from `AppDelegate.applicationDidFinishLaunching → setupVoiceModel()`**: it preloads at launch when the toggle is on, and reloads on the `.voiceModelDidChange` notification — it does not depend on any controller being activated in a text field first (`PromptInputController.reloadVoiceModel()` is the app-level entry point). Recording, RMS/length guards, sound effects, and the `onTranscription` callback are engine-agnostic.
* Model I/O (verify after conversion): input `encoder_input (1,560,1,T)` fp16, output `ctc_logits (1,25055,1,T)` fp16; `T` padded to one of `{21,38,54,88,171,254,338,504}` (≈1s–30s; longer audio is truncated).
* Offline regression: `SenseVoiceSmall/SenseVoiceKit/` (SwiftPM CLI; sources symlink the two app files as the single source of truth). `swift run sensevoice-cli <dist> <16k.wav>`.
...
* `AppSettings.voiceModelLoadState: VoiceModelLoadState` (`.notConfigured` / `.loading` / `.loaded` / `.failed`) is updated by `VoiceRecorder` and observed by `VoiceSettingsView` via `NotificationCenter`

* Model reload is triggered by `voiceModelDidChange` notification (posted when user clicks "应用" in Settings)

**Model setup:** The model ships **inside the app** — there is no path to paste. Before building, run `SenseVoiceSmall/build_sensevoice_mlmodelc.sh` to produce `SenseVoiceSmall/dist/` (containing `SenseVoiceSmall.mlmodelc`, `query_embeddings.f32`, `am.mvn`, `tokens.json`); the Xcode "Copy SenseVoice Model" Run Script phase then copies it into `Resources/SenseVoiceModel/`. If `dist/` is absent at build time the phase emits a warning and the app builds without voice support — the 语音输入 Settings pane shows a toggle plus "此版本未包含语音模型…" (`VoiceSettingsView` gates on `AppSettings.isVoiceModelBundled`) instead of any path field. Loading runs on `voiceQueue` to avoid blocking the main thread.

Because the model is a read-only bundle resource, it needs **no filesystem entitlement under App Sandbox** — this is why bundling (rather than reading a user-chosen path) is the sandbox-friendly design.
...
* `static let sharedVoiceRecorder`, observer (`voiceModelObserver`) — guarded with `guard Self.x == nil`
...
* `rebuild.sh` does NOT delete the user lexicon by default (the `rm` lines are commented). To reset word frequencies, uncomment the **container-path** `rm` inside the script (`~/Library/Containers/hk.eduhk.inputmethod.Prompt/Data/Library/userlexicon.sqlite3*`); do not delete the pre-sandbox `~/Library/userlexicon.sqlite3`, which is the one-time migration source.
