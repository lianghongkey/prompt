...
9. **默认输入模式可配 + 按输入框记忆** — 设置中可选择默认进入「中文」或「英文」；新输入框使用配置的默认值，同一个输入框失焦再回来会保留上次的中英文状态
...
* 任何状态下单击左 Shift 切到英文（ABC 直通），单击右 Shift 切回中文
* 在中文模式下若已经输入了一些拼音但还没选词，单击左 Shift 会把当前拼音作为英文原样上屏，并切换到英文模式
* **按输入框记忆**：在同一个输入框中切换出去（点到别处再回来）会保留上次切到的中英文状态；只有在新的输入框首次输入、或切换到其它输入法再切回来时，才会回到「设置 → 切换到本输入法时默认模式」中配置的默认值（默认是中文）
