...
2. **语音输入（完全本地、开箱即用）** — 内置 SenseVoiceSmall 模型（Core ML + 纯 Swift 推理），按一个组合键即可语音转文本
   - 模型随 App 打包，无需下载或配置路径
   - 延迟低，无隐私风险（不联网、不依赖任何第三方运行时）
3. **Shift 单击切换中英文** — 单击左 Shift → 英文（ABC 直通）；单击右 Shift → 中文
4. **造词功能** — 通过选择单字组合自定义词组，自动入库并按使用频率排序
5. **交叉引用过滤** — 在已有候选词的情况下按住 Shift 输入辅助词的拼音，把候选缩小为某个共同音节位置上能与辅助词组合的字
6. **上下文感知标点** — 中文标点模式下，紧跟在 ASCII 字母 / 数字后输入的标点自动变为半角；输入中文、切到中文模式、键盘移动光标或切换焦点之后立即恢复全角；字母与数字可分别开关
7. **删除选中的候选词** — `Shift+Space` 可从用户词库中删除当前高亮的候选词
8. **中英文模式按 App 记忆 + 默认模式** — 同一个 App 上次切换的中／英文状态会被记住，下次激活恢复；新 App 用默认模式；Shift 单击随时手动切换
...
| Shift+Space | 候选词显示中 | 从用户词库中删除当前高亮的候选词（仅当其来自用户词库时生效） |
| Shift+Space | 中文模式 / 待机 | 启动语音输入（需在设置中开启语音识别并等待模型加载完成） |
...
语音识别使用内置的 **SenseVoiceSmall** 模型（Core ML），推理全部在本进程内完成，纯 Swift 实现，不联网、不依赖 Python 或任何第三方运行时。

前置条件：在「设置 → 语音输入」中打开「启用语音识别（SenseVoice）」。模型已随 App 打包，**无需粘贴任何路径**。

> 首次开启会为神经引擎（ANE）编译优化模型，约需 1–2 分钟，这是一次性的；完成后状态变为「已加载」，之后每次启动约 1 秒。

使用方法：
1. 在中文模式下，**不要**正在输入拼音（buffer 为空）
2. 按住 `Shift+Space`：开始录音，光标处显示 💬 标记，并播放 "Tink" 提示音
3. 松开任一键：停止录音，播放 "Pop" 提示音；识别完成后文本自动上屏（繁体会自动转换为简体）

过滤规则：录音短于约 0.5 秒（8000 采样点）或音量 RMS ≤ 0.001 的片段直接丢弃，不做识别，也不上屏。

若某个构建未包含模型（构建前未生成 `SenseVoiceSmall/dist/`），设置页会提示「此版本未包含语音模型」，语音功能不可用。生成模型的方法见 `SenseVoiceSmall/README.md`。
...
在「中文标点」模式下，标点会根据**前一段输入**自动选择全/半角；默认开启，可在「设置 → 通用」里分别关闭「字母后使用英文标点」和「数字后使用英文标点」。

* 默认全角；输入 CJK 字符后保持/恢复全角
* 输入 ASCII 字母或数字后切换到半角
* 重置回全角的时机：
  * 切换焦点（焦点切到别的输入框 / 别的 App）或重新激活输入法
  * 通过 Shift 单击 / Caps Lock 切换到中文模式
  * 用键盘移动光标：方向键、Home、End、PageUp、PageDown（即使带 Cmd / Option / Shift 修饰也算）
* 不会重置的情况：用鼠标点击移动光标。系统不会把鼠标点击事件传给输入法，所以这种情况下需要按一次方向键或随便输入一个汉字来让它回到全角

### 删除候选词（Shift+Space）

当候选词区显示且当前高亮的候选词来自用户词库时，按 `Shift+Space` 可将其从用户词库中删除。如果高亮的候选词不是用户词库条目，则无操作。
...
* **输入法选择**（中文 / 英文）
  ...
  * 不记忆输入法状态的 App 列表
* **语音输入**
  * 启用语音识别（SenseVoice）；显示模型加载状态（未配置 / 加载中 / 已加载 / 失败）
* **模糊音**（zh↔z、ch↔c、sh↔s、n↔l 等）
* **顺序纠错**（拼写顺序的单向纠错，例如 `gn → ng`、`na → an`、`ne → en`，仅当原始拼音匹配不到时作为额外候选追加，反向不成立）
* **关于**

字符标准（简 / 繁）目前只能通过 defaults 修改。由于 App 已启用沙箱，UserDefaults 位于容器内，必须**按路径**写入容器的偏好设置文件（写全局域无效）：

```bash
PREFS=~/Library/Containers/hk.eduhk.inputmethod.Prompt/Data/Library/Preferences/hk.eduhk.inputmethod.Prompt

# 切换到简体
defaults write "$PREFS" CharacterStandard -int 4

# 切换到繁体
defaults write "$PREFS" CharacterStandard -int 1
...
> `rebuild.sh` **默认保留**用户词库（脚本里清库的 `rm` 行是注释掉的）。如需重置词频，取消脚本中**容器路径**那一行的注释。
...
* Python 3（用于词典扩展脚本，以及 SenseVoice 模型转换脚本）
...
* **SenseVoiceSmall/** — 语音模型转换脚本与离线回归 CLI，产物 `dist/` 在构建时被拷进 App
...
```bash
./SenseVoiceSmall/build_sensevoice_mlmodelc.sh
```

生成 `SenseVoiceSmall/dist/`（`SenseVoiceSmall.mlmodelc`、`query_embeddings.f32`、`am.mvn`、`tokens.json`），Xcode 的 "Copy SenseVoice Model" 构建阶段会把它拷入 App 的 `Resources/SenseVoiceModel/`。若 `dist/` 不存在，构建仍会成功，但产物不含语音功能。
...
（旧路径 `~/Library/userlexicon.sqlite3` 现在只作为首次启动时的一次性迁移来源。）
