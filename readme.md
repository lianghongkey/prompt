...
7. **上下文感知标点** — 中文标点模式下，紧跟在 ASCII 字母 / 数字后输入的标点自动变为半角；输入中文、切到中文模式、键盘移动光标或切换焦点之后立即恢复全角；可在设置中关闭
...
在「中文标点」模式下，标点会根据**前一段输入**自动选择全/半角；默认开启，可在「设置 → 通用 → 数字 / 字母后使用英文标点」关闭。
